...
describe('Turn', function() {
  ...
  it('should return a guess', function() {
    const card = new Card(7, 'What is the fastest animal on Earth', ['road runner', 'cheetah', 'elephant'], 'cheetah');
...
//why does this work with eql but not equal??
  ...
  it('should evaluate guess', function() {
    const card = new Card(13, 'What is Cindy\'s dog\'s name?', ['Alfredo', 'Bonnie Rae', 'Waylon'], 'Bonnie Rae');
    const turn = new Turn('Alfredo', card);
    expect(turn.evaluateGuess()).to.equal(false);
  });

  it('should give feedback', function() {
    const card = new Card(34, 'What pet do I want for my birthday?', ['bunny', 'puppy', 'bird'], 'bunny');
