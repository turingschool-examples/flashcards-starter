...
describe('Round', function() {
    ...
    it('should switch to a new card once a guess is made', function() {
        round.takeTurn()

        assert.equal(deck.cards[2], deck.cards[2])
    })

    it('should update the incorrectGuesses property with the card id when guess is wrong', function() {
        round.takeTurn('purple');
        round.takeTurn('blue');
        round.takeTurn('yellow');

        assert.equal(round.incorrectGuesses.length, 3)
    })

    it('should have a method that calculates the percentage the player has guessed correctly', function() {
        round.takeTurn('purple');
        round.takeTurn('blue');
        round.takeTurn('yellow');
